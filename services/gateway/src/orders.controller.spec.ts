...
describe('Gateway Orders REST', () => {
  ...
  beforeAll(async () => {
    const moduleRef: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(OrdersClient)
      .useValue(ordersClientMock)
      .overrideProvider(ReceiptClient)
      .useValue(receiptClientMock)
      .compile();
  ...
  it('GET /v1/orders/:id → 404 cuando gRPC responde NOT_FOUND', async () => {
    ordersClientMock.call.mockRejectedValue({
  ...
  it('GET /v1/orders/:id/receipt → 200 cuando receipt responde bien', async () => {
    receiptClientMock.call.mockResolvedValue({
      order_id: 'ORD-1',
      total: 50,
    });

    const res = await request(app.getHttpServer())
      .get('/v1/orders/ORD-1/receipt');
